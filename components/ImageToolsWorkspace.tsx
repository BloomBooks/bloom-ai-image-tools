...
import { editImageWithGoogle } from "../services/googleAiService";
...
export function ImageToolsWorkspace({
  ...
  const isGoogleActive = !effectiveApiKey && !!googleApiKey;
  const hasUsableKeyForActiveProvider = !!effectiveApiKey || !!googleApiKey;
  // Non-Google models cannot run through the Google provider.
  const disabledModelIds = useMemo(
    () =>
      isGoogleActive
        ? MODEL_CATALOG.filter((model) => !isGoogleModel(model.id)).map((model) => model.id)
        : [],
    [isGoogleActive],
  );
  ...
  const openRouterStatusLabel = effectiveApiKey
    ? usingEnvKey
  ...
  const refreshCredits = useCallback(async () => {
    ...
    // Only OpenRouter exposes a credits/balance endpoint. When no OpenRouter key
    // is present (e.g. Google mode), there is nothing to fetch; the header shows a
    // static "Google AI Studio" badge instead.
    if (!effectiveApiKey) {
      setCredits(null);
  ...
  useEffect(() => {
    setState((prev) => ({
      ...prev,
      isAuthenticated: !!(apiKey || envApiKey || googleApiKey),
    }));
  }, [apiKey, envApiKey, googleApiKey]);

  // When the Google provider is active, keep a Google model selected.
  useEffect(() => {
    if (isGoogleActive && !isGoogleModel(selectedModelId)) {
      setSelectedModelId(DEFAULT_GOOGLE_MODEL?.id || selectedModelId);
    }
  }, [isGoogleActive, selectedModelId]);
  ...
  }, [isHydrated, hasUsableKeyForActiveProvider, canUseSelectedModelWithoutApiKey]);
    ...
    void (async () => {
      ...
      try {
        ...
        if (persisted) {
          ...
          if (persisted.auth?.googleApiKey) {
            setGoogleApiKey(persisted.auth.googleApiKey);
          }
  ...
  const googleApiKeyRef = useRef(googleApiKey);
  ...
  useEffect(() => {
    googleApiKeyRef.current = googleApiKey;
  }, [googleApiKey]);
  useEffect(() => {
    ...
    const buildPersistableState = () => {
      ...
      return {
        ...
        auth: {
          ...
          googleApiKey: googleApiKeyRef.current,
        },
  ...
  const handleApplyTool = async (toolId: string, params: Record<string, string>) => {
    ...
    try {
      ...
      if (usesLocalBackgroundRemoval) {
        ...
        const useGoogle = isGoogleActive;
        const resolvedApiKey = useGoogle ? googleApiKey || "" : effectiveApiKey;
        ...
        if (!resolvedApiKey && !canRunWithoutApiKey) {
          ...
          setState((prev) => ({
            ...
            isProcessing: false,
            error: useGoogle
              ? "Add a Google AI Studio API key before running tools."
              : "Connect to OpenRouter before running tools.",
          }));
          return;
        }

        // The Google provider has no credits endpoint to refresh.
        shouldRefreshCredits = !canRunWithoutApiKey && !useGoogle;
        ...
        const result = useGoogle
          ? await editImageWithGoogle(sourceImages, prompt, resolvedApiKey, selectedModel?.id, {
              signal: abortController.signal,
              imageConfig,
              reasoningLevel: reasoningLevelForRequest,
            })
          : await editImage(sourceImages, prompt, resolvedApiKey, modelIdForRequest, {
              signal: abortController.signal,
              imageConfig,
              reasoningLevel: reasoningLevelForRequest,
            });
  ...
  const handleProvideGoogleKey = (key: string) => {
    const trimmed = key.trim();
    setGoogleApiKey(trimmed || null);
  };
  ...
  const shouldShowConnectToOpenRouterCTA =
    !hasUsableKeyForActiveProvider && !canUseSelectedModelWithoutApiKey;
  const connectCtaLabel = "Connect to OpenRouter";
    ...
    <ThemeProvider theme={darkTheme}>
      ...
      >
          ...
          <Stack spacing={1} alignItems="flex-end" sx={{ flexShrink: 0 }}>
            <Stack direction="row" spacing={3} alignItems="center">
              <OpenRouterCreditsHeader
                ...
                connectCtaLabel={connectCtaLabel}
                isGoogleProvider={isGoogleActive}
        ...
        <AIImageToolsSettingsDialog
          ...
          google={{
            apiKeyPreview: googleApiKey,
            onProvideKey: handleProvideGoogleKey,

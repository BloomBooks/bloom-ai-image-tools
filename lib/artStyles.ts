...
export const getArtStylesByCategories = (
  categories?: string | string[],
  options?: ArtStyleFilterOptions,
): ArtStyle[] => {
  const excludeNone = options?.excludeNone ?? false;
  const excludedIds = new Set((options?.excludeIds ?? []).map((id) => id?.trim()).filter(Boolean));
  ...
  if (excludedIds.size > 0) {
    result = result.filter((style) => !excludedIds.has(style.id));
  }

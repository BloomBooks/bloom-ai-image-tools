 ...
 */
export type MagnifiableImageProps = React.ImgHTMLAttributes<HTMLImageElement> & {
  zoom?: number;
  lensSize?: number;
...
export const MagnifiableImage = React.forwardRef<HTMLImageElement, MagnifiableImageProps>(
  (props, forwardedRef) => {
    const {
      zoom = 2.25,
      lensSize = 160,
    ...
    const updateLensPosition = React.useCallback(
      (clientX: number, clientY: number) => {
        ...
        if (offsetX < 0 || offsetY < 0 || offsetX > imageRect.width || offsetY > imageRect.height) {
          setLensVisible(false);
          return;
        }
